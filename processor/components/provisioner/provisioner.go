...
func (provisioner *Provisioner) AddRemoteModules(folder string) error {
	...
	filepath.WalkDir(folder, func(path string, d fs.DirEntry, err error) error {
		...
		return provisioner.AddRemoteModule(path)
	})

	return nil
}

func (provisioner *Provisioner) AddRemoteModule(folder string) error {
	...
	return provisioner.AddModule(local)
}
...
func (provisioner *Provisioner) AddModule(implementation *module.Module) error {
	...
	for _, export := range implementation.Config.Exports {
		...
		clusterWrapper.DefaultConfig = cluster.Config{
